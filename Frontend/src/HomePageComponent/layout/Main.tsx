...
// Helper function to check if user is logged in by checking token in cookies
...
function Main() {
  ...
  const handleAddToCart = async () => {
    ...
    if (!isUserLoggedIn()) {
      const userChoice = confirm("Please login or register to add items to basket.\n\nClick OK to go to login page.");
      if (userChoice) {
        navigate("/login");
      }
      return;
  ...
  return (
    <main className="flex flex-col gap-10">
          ...
          >
            {food.map((item) => (
              <SwiperSlide key={item._id}>
                <div
                  onClick={() => setSelectedFood(item)}
                  className="bg-white rounded-2xl p-3 shadow-md flex flex-col h-full cursor-pointer hover:shadow-lg transition"
                >
                  <div className="w-full h-56 rounded-2xl overflow-hidden bg-gray-100">
                    <img
                      src={item.image?.url}
        ...
        {selectedFood && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-xl font-bold">{selectedFood.name}</h2>
              <p className="text-gray-600 mt-1">{selectedFood.description}</p>
              <p className="text-2xl font-bold text-orange-500 mt-2">Rs {totalPrice.toFixed(2)}</p>
              ...
              {/* Add to cart button with token check */}
              <button
              ...
              >
                {loading ? "Adding..." : (isLoggedIn ? "Add to Basket" : "Login to Order")}
              </button>
      ...
      <div className="flex flex-col items-center gap-8 px-4 md:px-6 lg:px-10">
        ...
        <button className="bg-[#FF9A0E] text-white font-bold px-6 py-2 rounded-lg hover:bg-[#E58900] transition">
          View All
        </button>
      </div>
